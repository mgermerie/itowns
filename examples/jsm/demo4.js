...
function createWaterShaderMaterial() {
    ...
    return new THREE.ShaderMaterial({
        ...
        fragmentShader: `
            ...
            // === COLLER ICI TOUTES LES FONCTIONS DU SHADER SEASCAPE ===
            // hash(), noise(), map(), sea_octave(), getSeaColor(), getSkyColor(), getPixel(), mainImage(), etc.
            /*
...
async function addWaterFromConfig(configUrl) {
    ...
    const { coordinates, dimensions, color } = config;
    const water = new WaterMesh({ coordinates, dimensions, color });
    ...
    view.addFrameRequester(itowns.MAIN_LOOP_EVENTS.BEFORE_RENDER, () => {
        ...
        view.scene.traverse((obj) => {
            if (obj instanceof WaterMesh) {
                obj.uniforms.iTime.value = elapsedTime;
            }
        });
    });
